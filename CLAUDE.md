...
- Left-rail reference HTML — `demo/sidebar-worktree-band-v3.html` is the canonical rail reference for the **v3** body (default). Shared chrome, top to bottom: a scope control (a project / `Active now`) → an attention **filter bar** (`All` + Needs you / Unread / Working / Idle, neutral counts) + a `New session` launcher → the variant body (see below) → a **profile footer**. `Add project` lives in the scope dropdown. The rail ships a second, interchangeable body variant, picked in Settings → Web → Sidebar (`wos.sidebar.variant` localStorage key, defaults to v3):
  - **v3 (default)** — a live-session **stream** grouped by attention (project identity **tiles**, the working state animates the tile border; no per-row status dot — the group states the state) followed by a collapsible **Worktrees band**.
  - **v4** — canonical `demo/sidebar-worktree-tree-v4.html`: a worktree **tree** — each worktree is an expandable node whose live sessions are its children, open by default. A manual collapse is remembered per-device (`wos.sidebar.tree.collapsed`) and stays sticky — new attention arriving later does not reopen it. No health dot, status word, or session count on the worktree row.
  - Implementation: `apps/web/src/components/sidebar.tsx` (shared chrome + `useSidebarController`), `sidebar-v3-body.tsx`, `sidebar-v4-body.tsx`. Do not delete either demo file; keep the matching body in sync with it.
