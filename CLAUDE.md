...
- Project identity palette: `--p-1 … --p-8` (light + dark) — muted, distinct hues used only to color the rail's session-stream project tiles (`ProjectTile`, via `color-mix`). Deliberately excludes the status semantics (`--good` / `--warn` / `--bad`): they read as identity, not health. Assigned deterministically by `projectTile(project)` (`lib/project-identity.ts`); there is no per-project color picker.
